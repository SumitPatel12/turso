...
impl PageCache {
    ...
    pub fn resize(&mut self, new_cap: usize) -> CacheResizeResult {
        ...
        // If shrinking and we have more entries than new capacity, evict excess
        if new_cap < self.len() {
            // Evict entries one by one until we're at new capacity
            while self.len() > new_cap {
                // Try to evict one entry using SIEVE algorithm
                let old_cap = self.capacity;
                self.capacity = self.len(); // Temporarily set capacity to current size
                match self.make_room_for(1) {
                    Ok(()) => {
                        // Successfully freed up one slot
                    }
                    Err(_) => {
    ...
    pub fn make_room_for(&mut self, n: usize) -> Result<(), CacheError> {
        ...
        while need > 0 && examined < max_examinations {
            // If clock hand is null, start from front
            if self.clock_hand.is_null() {
                let front_cursor = self.queue.front_mut();
                if front_cursor.get().is_some() {
                    self.clock_hand =
    ...
    pub fn truncate(&mut self, max_page_num: usize) -> Result<(), CacheError> {
        let keys_to_delete: Vec<PageCacheKey> = self
            .map
            .keys()
